...
<span :class='[$style.title, $style.bold]'>1.重要极限</span>

（1）第一个重要极限：$\lim\limits_{x\to0}\frac{\sin x}x=1$。

（2）第二个重要极限：$\lim\limits_{x\to0}(1+x)^{\frac1x}=e$。

更一般地，有 $\lim\limits_{\Delta\to0}(1+\Delta)^{\frac1{\Delta}}=e$，$\lim\limits_{\varphi(x)\to0}[1+\varphi(x)]^{\frac1{\varphi(x)}}=e$。

:::tip 划重点
（1）当 $0\lt x\lt\frac\pi 2$ 时，有 $0\lt\sin x\lt x\lt\tan x$。

（2）记住如下常见的不等式：

① 当 $x\gt-1$ 时，有 $\ln(1+x)\leq x$，其中当且仅当 $x=0$ 时等号成立；

② $e^x\geq1+x$，其中当且仅当 $x=0$ 时等式成立。
:::

<span :class='[$style.title, $style.bold]'>2.当 $x\to0$ 时，常见的等价无穷小：</span>

（1）$x\sim\sin x\sim\tan x\sim\arcsin x\sim\arctan x\sim\ln(1+x)\sim e^x-1$。

（2）$1-\cos x\sim\frac12x^2$。
...
<span :class='$style.title'>1.函数在一点连续</span>——设函数 $f(x)$ 在 $x=a$ 的邻域内有定义，若 $\lim\limits_{x\to a}f(x)=f(a)$（或者 $f(a-0)=f(a+0)=f(a)$），则称函数 $f(x)$ 在 $x=a$ 处连续；

若 $f(a-0)=f(a)$，则称函数 $f(x)$ 在 $x=a$ 处左连续；若 $f(a+0)=f(a)$，则称函数 $f(x)$ 在 $x=a$ 处右连续。

<span :class='$style.title'>2.函数在闭区间上连续</span>——设函数 $f(x)$ 在闭区间 $[a,b]$ 内有定义，满足

（1）函数 $f(x)$ 在 $(a,b)$ 内的每一点处都连续；

（2）$f(a)=f(a+0)$，$f(b)=f(b-0)$，

则称函数 $f(x)$ 在闭区间 $[a,b]$ 内连续，记为 $f(x)\in C[a,b]$。

（二）间断

设函数 $f(x)$ 在 $x=x_0$ 的去心邻域内有定义，若 $\lim\limits_{x\to x_0}f(x)\neq f(x_0)$，则称函数 $f(x)$ 在 $x=x_0$ 处间断（不连续），$x=x_0$ 称为 $f(x)$ 的间断点。

设函数 $f(x)$ 在 $x=x_0$ 处间断，间断点的分类如下：

<span :class='$style.title'>1.第一类间断点</span>——若 $f(x_0-0)$，$f(x_0+0)$ 都存在，则称点 $x=x_0$ 为函数 $f(x)$ 的第一类间断点，其中若 $f(x_0-0)=f(x_0+0)$，则称点 $x=x_0$ 为 $f(x)$ 的可去间断点；

若 $f(x_0-0)\neq f(x_0+0)$，则称点 $x=x_0$ 为 $f(x)$ 的跳跃间断点。

<span :class='$style.title'>2.第二类间断点</span>——若 $f(x_0-0)$，$f(x_0+0)$ 至少有一个不存在，则称点 $x=x_0$ 为函数 $f(x)$ 的第二类间断点。

### 二、闭区间上连续函数的性质

<span :class='$style.title'>性质 1（最小值与最大值定理）</span> 设 $f(x)\in C[a,b]$，则 $f(x)$ 在 $[a,b]$ 上取到最小值 $m$ 和最大值 $M$。

<span :class='$style.title'>性质 2（有界定理）</span> 设 $f(x)\in C[a,b]$，则 $f(x)$ 在 $[a,b]$ 上有界，即存在 $M_0\gt0$，对一切的 $x\in[a,b]$，有 $|f(x)|\leq M_0$。

<span :class='$style.title'>性质 3（零点定理）</span> 设 $f(x)\in C[a,b]$，且 $f(a)f(b)\lt0$，则至少存在一点 $c\in(a,b)$，使得 $f(c)=0$。

<span :class='$style.title'>性质 4（介质定理）</span> 设 $f(x)\in C[a,b]$，$m$ 和 $M$ 分别为 $f(x)$ 在 $[a,b]$ 上的最小值和最大值，则对任意的 $\eta\in[m,M]$，至少存在一点 $\xi\in[a,b]$，使得 $f(\xi)=\eta$

## 本章重要考点及题型

### <span :class="$style.title">题型一 不定型极限计算</span>

<span :class='$style.title'>情形 1</span> $\frac00$ 型

对于 $\frac00$ 型的极限，常用等价无穷小、洛必达法则、泰勒公式等方法，需要做如下一些说明：

（1）若函数表示式中出现 $u(x)^{v(x)}$，一般化为 $e^{v(x)\ln u(x)}$，当 $v(x)\ln u(x)\to0$ 时，

$$
e^{v(x)\ln u(x)}-1\sim v(x)\ln u(x)
$$

（2）若函数表达式中出现 $\ln\varphi(x)$，一般化为 $\ln[1+(\varphi(x)-1)]$，当 $\varphi(x)\to0$ 时，

$$
\ln[1+(\varphi(x)-1)]\sim\varphi(x)-1
$$

（3）若函数表达式中出现 $f(x)-1$，一般通过变形后利用 $\varphi(x)\to0$ 时

$$
e^{\varphi(x)}-1\sim\varphi(x),或[1+\varphi(x)]^a-1\sim a\varphi(x)
$$

<span :class='$style.title'>情形 2</span> $1^\infty$ 型

计算 $1^\infty$ 型的极限可进行如下步骤：

（1）凑第二个重要极限形式 $(1+\Delta)^\frac1\Delta$；

（2）保持恒等变形。

<span :class='$style.title'>情形 3</span> $\frac\infty\infty$ 型

计算 $\frac\infty\infty$ 型的极限一般有如下思路：
...
① $\lim\limits_{x\to+\infty}\frac{\ln^ax}{x^b}=0(a\gt0,b\lt0)$；

② $\lim\limits_{x\to+\infty}\frac{x^a}{b^x}=0(a\gt0,b\gt1)$。

（4）$\lim\limits_{x\to\infty}\frac{b_nx^n+\cdots+b_1x+b_0}{a_mx^m+\cdots+a_1x+a_0}(a_m\neq0,b_n\neq0)=\begin{cases}0,\quad&n\lt m\\\frac{b_n}{a_m},&n=m\\\infty,&n\gt m\end{cases}$

<span :class='$style.title'>情形 4</span> $\infty\cdot0$ 型

对于 $\infty\cdot0$ 型的不定型极限，通常有如下思路：

（1）化为 $\frac00$ 型或 $\frac\infty\infty$ 型极限；

（2）利用中值定理。

<span :class='$style.title'>情形 5</span> $\infty-\infty$ 型

$\infty-\infty$ 型极限一般作如下处理：

（1）若有分母一般先通分；

（2）无分母时，一般对分子有理化、提取、分组等。

### <span :class="$style.title">题型二 利用夹逼定理求极限</span>

### <span :class="$style.title">题型三 数列极限存在性证明</span>

证明数列极限存在一般转化为证明数列单调有界，具体来说分为：

（1）若数列 ${a_n}$ 单调递减，只要证明数列 ${a_n}$ 有上界；

（2）若数列 ${a_n}$ 单调递增，只要证明数列 ${a_n}$ 有下界。

### <span :class="$style.title">题型四 无穷小的比较</span>

### <span :class="$style.title">题型五 闭区间上连续函数性质问题</span>

### <span :class="$style.title">题型六 函数的连续性、间断点及分类</span>
