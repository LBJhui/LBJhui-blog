...
- 注释<br />
  ❑ 单行注释：`//`<br />
  ❑ 多行注释：`/* */`
- [严格模式 `"use strict";`](../1.严格模式.md)<a href="https://blog.csdn.net/qq_36081714/article/details/156187914" v-if="false">严格模式</a>
- 语句
...
<h2><a href="https://blog.csdn.net/qq_36081714/article/details/111663674" v-if="false">变量</a></h2>
...
### [3.4.2 Undefined 类型](../3.你不知道的undefined.md)
...
:::details null 和 undefined 的区别？

| 对比维度             | `undefined`                                      | `null`                                              |
|----------------------|--------------------------------------------------|-----------------------------------------------------|
| 数据类型         | 原始类型（primitive）                            | 原始类型（primitive）<br>（⚠️ `typeof null` 返回 `"object"` 是历史 bug） |
| 语义含义         | “未定义”或“缺失的值”<br>（系统自动赋予）         | “有意为空”或“空引用”<br>（开发者显式赋值）          |
| 数值转换结果     | `Number(undefined)` → `NaN`                      | `Number(null)` → `0`                                |
| 布尔转换结果     | `Boolean(undefined)` → `false`                   | `Boolean(null)` → `false`                           |
| 相等性（==）     | `null == undefined` → `true`                     | `undefined == null` → `true`                        |
| 相等性（===）    | `null === undefined` → `false`                   | `undefined === null` → `false`                      |
| 典型出现场景     | 1. 声明未赋值的变量<br>2. 函数参数未传入<br>3. 访问对象不存在的属性<br>4. 函数无返回值 | 1. 显式表示“无对象”<br>2. DOM 方法未找到元素（如 `getElementById`）<br>3. 原型链终点（`Object.getPrototypeOf(Object.prototype) === null`）<br>4. 主动释放引用（内存管理） |
| 是否应由开发者赋值 | ❌ 不推荐显式赋值为 `undefined`                  | ✅ 推荐用 `null` 表示“空”或“无”                    |
| 在 API 设计中的用途 | 表示“未提供”或“使用默认值”                      | 表示“明确不要”或“清空”                              |
...
let bar = new Bar()
console.log(3 + bar) // '3defaultbar' +运算符触发默认模式，hint被设置为 "default"
console.log(3 - bar) // 0
...
ECMAScript 只要求在给构造函数提供参数时使用括号。 `let o = new Object // 合法，但不推荐`
...
console.log(a.propertyIsEnumerable('c')) // false
```
