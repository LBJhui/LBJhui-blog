...
- strictNullChecks:true(strictNullChecks 标记的启用是在 tsconfig.json 文件里进行配置)，在严格的 null 检查模式下，null 和 undefined 值不包含在任何类型里，只允许赋值给 void 和本身对应的类型。
...
// 当访问 str.length 时，JavaScript 引擎做了以下⼯作：
let size = (function () {
  // 1. ⾃动装箱：创建⼀个临时的 String 对象包装原始字符串
...
// 没有明确的表示 b 的类型是 any，但TS主动推断出来 b 是 any —— 隐式的 any
let b
// 以下对 b 的赋值，均⽆警告
b = 100
b = '你好'
...
unknown 的含义是：未知类型，适用于：起初不确定数据的具体类型，要后期才能确定。unknown 任何类型的值都可以赋值给 unknown，unknown 只能赋值给 unknown、any
...
```typescript
// 定 a 的类型为 never，那就意味着 a 以后不能存任何的数据了
let a: never
...
if (typeof a === 'string') {
  ...
  console.log(a) // TypeScript会推断出此处的 a 是 never，因为没有任何⼀个值符合此处的逻辑
}
```
...
```typescript
// 限制 throwError 函数不需要有任何返回值，任何值都不⾏，像 undefined、null 都不⾏
...
void 的含义是空，即：函数不返回任何值，调⽤者也**不应依赖其返回值进行任何操作**！
...
官⽅描述：所有可以调用 Object 方法的类型。简单记忆：除了 undefined 和 null 的任何值。由于限制的范围实在太大了！所以实际开发中使用频率极低。

```typescript
let b: Object // b 的值必须是 Object 的实例对象（除去 undefined 和 null 的任何值）
...
// 如下赋值不合法，因为 person2 的类型限制中，没有对 gender 属性的说明
...
```typescript
// 限制 person 对象必须有 name 属性，可选 age 属性但值必须是数字，同时可以有任意数量、任意类型的其他属性
let person: {
  name: string
  age?: number
...
```typescript
// PersonInterface 接口，⽤与限制 Person类 的格式
