...
选择器就是根据不同的需求把不同的标签选出来，这就是选择器的作用。简单来说，就是选择标签用的。
...
- 找到所有的 h1 标签。（选对人）
- 设置这些标签的样式：字体颜色为红色、字体大小为 26 像素。（做对事）
...
- 元素 1 是父级，元素 2 是子级，最终选择的是元素 2，即：元素 1 是不会生效样式的
- 元素 2 可以是儿子，也可以是孙子等，只要是元素 1 的后代即可
- 元素 1 和元素 2 **可以是任意基础选择器**

```html
<style>
  /* 把 ol 里面的 li 选出来改为 pink */
  ol li {
    color: pink;
  }

  /* 把 ol 里面的 a 选出来改为 red */
  ol a {
    color: red;
  }

  /* 把 ul 里面的 li 选出来改为 green */
  ul li {
    color: green;
  }
...
<style>
  ...
  /* 2. 选择 ul 里面的最后一个孩子 li */
  ul li:last-child {
    background-color: pink;
  }
  /* 3. 选择 ul 里面的第 2 个孩子 li */
  ul li:nth-child(2) {
    background-color: skyblue;
  }
  /* 4. 选择 ul 里面的第 6 个孩子 li */
  ul li:nth-child(6) {
    background-color: skyblue;
  }
</style>
...
`:focus-within` 伪类选择器表示当元素或其任意后代元素被聚焦时，将匹配该元素。
...
<style>
  @font-face {
    ...
    src:
      url('fonts/icomoon.eot?1lv3na#iefix') format('embedded-opentype'),
...
| ------------------ | ---------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------- |
| **语法标识**       | 单冒号 `:`（如 `:hover`、`:focus`）                                          | 双冒号 `::`（如 `::before`、`::after`；CSS2 中曾用单冒号，现代建议双冒号区分）                |
| **核心作用**       | 选择**处于特定状态/位置**的已有元素                                          | 创建**元素的某个虚拟部分**（DOM 中不存在），并为其设置样式                                    |
| **是否生成新元素** | 不生成，仅为已有元素添加状态                                                 | 生成虚拟元素（属于原元素的子元素/部分）                                                       |
| **样式作用对象**   | 作用于整个选中的元素                                                         | 作用于生成的虚拟元素（可设置宽高、内容等）                                                    |
| **常见示例**       | `:hover`、`:focus`、`:active`、`:nth-child()`、`:focus-visible`、`:disabled` | `::before`、`::after`、`::first-line`、`::first-letter`、`::selection`                        |
| **是否可嵌套**     | 支持（如 `a:hover:focus` 选中“悬停且聚焦的链接”）                            | 不支持嵌套，且一个元素通常仅能使用一个伪元素（如同时用::before和::after是允许的，但不能嵌套） |
| **content 属性**   | 无意义（设置后不生效）                                                       | 必须配合 `content` 属性（即使值为空 `content: ""`），否则伪元素不显示                         |

- 伪类
  - 状态类：`:link` `:visited` `:hover` `:active` `:focus` `:paused`
...
- 伪元素
  ...
  - `::selection`
  - `::placeholder`
  - `::backdrop`：是一个与视口大小相同的盒子，它会被渲染在任何顶层展示元素的下方。
...
CSS 选择器的优先级（Specificity）决定了当多个规则应用于同一个元素时，哪一个样式最终会生效。

随着 CSS 的发展，优先级的计算已经从简单的“权重比大小”演变成了包含**层叠层（Cascade Layers）**的复杂机制。

### CSS 优先级层级

| 排名     | 层级/类型                   | 说明                     | 备注                                                                                                         |
| :------- | :-------------------------- | :----------------------- | :----------------------------------------------------------------------------------------------------------- |
| **NO.1** | **`!important` + 层上下文** | 带有 `!important` 的声明 | **注意**：在 `@layer` 中，`!important` 的规则与普通规则相反（详见下文）。                                    |
| **NO.2** | **内联样式**                | `style="..."`            | 权重记为 `1000`，高于任何选择器。                                                                            |
| **NO.3** | **未分层样式**              | 普通 CSS 代码            | **关键点**：任何**不在** `@layer` 中的普通样式，优先级都**高于**所有 `@layer` 中的样式（无论层内权重多高）。 |
| **NO.4** | **@layer 层叠层**           | 后定义的层 > 先定义的层  | 层内遵循常规权重规则。                                                                                       |
| **NO.5** | **源次序**                  | 后定义的 > 先定义的      | 权重相同时，写在后面的覆盖前面的。                                                                           |
...
#### 2. @layer 层叠层 (CSS Cascade Layers)

这是现代 CSS 架构（如 Tailwind, Bootstrap 5+）的核心。它允许你**打破权重规则**。

- **规则**：**后声明的层 > 先声明的层**。
- **压制**：即使“后声明层”里只是一个简单的标签选择器（如 `div`），它也能覆盖“先声明层”里的 `!important`（普通模式下）或超高权重 ID 选择器。

**代码示例：**

```css
/* 1. 定义层的顺序：theme 优先级 < utility */
@layer theme, utility;
...
/* 结果：文字是蓝色的。因为 utility 层 > theme 层，层级优先于权重。 */
```

**⚠️ 关于 `!important` 在层中的反转：**
在 `@layer` 内部，`!important` 的优先级是**反转**的。

- 普通 CSS：后写的 `!important` 赢。
- `@layer` 中：**先声明的层**中的 `!important` 赢。
  - 例如：`@layer A, B;`。如果 A 和 B 都有 `!important`，**A 胜出**。

#### 3. 未分层样式 (Unlayered Styles)

这是最容易踩坑的地方。**没有放入 `@layer` 的普通 CSS 代码，被视为一个“隐含的最高优先级层”**（仅次于内联样式）。
...
/* 即使这个选择器权重很低，它也会覆盖上面的层内样式 */
button {
  background: blue;
}
```

## 15.[@scope](https://www.zhangxinxu.com/wordpress/2024/01/css-at-scope/)
