<script setup>
import grid from './components/1/网格布局中的动画.vue'
</script>
...
当你为网格容器添加 display: grid 样式时，只有直接子元素会成为网格项，这些网格项随后可被放置在你创建的网格中，而这些网格项的子元素仍会以常规流布局展示。
...
`subgrid` 子网格设置 `height` 高度是无效的，其高度只能继承自父网格元素。

<div style="display: grid; gap: 1rem; grid-template-columns: repeat(auto-fill, minmax(200px, 1fr));margin-bottom:20px">
  <section style="display: grid; grid-template-rows: subgrid; grid-row: span 4; gap: 0.5rem; padding: 0.75rem; background: #f0f3f9">
    <h4 style="background-color: #333; color: #fff">标题</h4>
    <p>作者</p>
    <blockquote style="background-color: skyblue">子网格</blockquote>
    <footer>3人参与 11月24日</footer>
  </section>
  <section style="display: grid; grid-template-rows: subgrid; grid-row: span 4; gap: 0.5rem; padding: 0.75rem; background: #f0f3f9">
    <h4 style="background-color: #333; color: #fff">标题</h4>
    <p>作者</p>
    <blockquote style="background-color: skyblue">Lorem ipsum dolor sit amet</blockquote>
    <footer>3人参与 11月24日</footer>
  </section>
  <section style="display: grid; grid-template-rows: subgrid; grid-row: span 4; gap: 0.5rem; padding: 0.75rem; background: #f0f3f9">
    <h4 style="background-color: #333; color: #fff">标题</h4>
    ...
    <blockquote style="background-color: skyblue">
      Lorem, ipsum dolor sit amet consectetur adipisicing elit. Quo eligendi repudiandae error molestias consequatur reprehenderit labore cum dolores iure ab?
    </blockquote>
    <footer>3人参与 11月24日</footer>
  </section>
</div>
...
<div class="container">
  ...
  <section class="item">
    <h4>标题</h4>
    <p>作者</p>
    <blockquote>Lorem, ipsum dolor sit amet consectetur adipisicing elit. Quo eligendi repudiandae error molestias consequatur reprehenderit labore cum dolores iure ab?</blockquote>
    <footer>3人参与 11月24日</footer>
  </section>
</div>
```
...
首先，第一个 `subgrid` 会继承祖先 `Grid` 布局的 `gap` 计算值大小。

例如，上面的案例，如果 `.item` 元素如果不设置 `gap:.5rem`，那么其就会按照父元素的 `gap:1rem` 进行渲染。

其次，`subgrid` 需要指定跨度范围。

我们可以在 `subgrid` 子网格元素上，使用 `grid-row` 或 `grid-column`（看 `subgrid` 设置的方向）属性设置。

如果子网格元素的子项横跨多个横或者列，则需要在子网格元素的子元素上使用 `grid-row` 或 `rid-column` 属性进行设置。
...
将多个元素的 `grid-area` 设置为一样可以使元素重叠，可以用来代替 `position: absolute` 解决布局问题。
